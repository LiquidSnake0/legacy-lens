...
public class SdkStyleConversion
{
    ...
    /// <summary>
    /// Targets generated by tooling rather than written by anyone.
    ///
    /// `EnsureNuGetPackageBuildImports` exists to fail the build when one of
    /// the `..\packages\` imports below is missing. Under PackageReference
    /// there are no such imports and nothing for it to guard.
    /// </summary>
    private static readonly HashSet<string> GeneratedTargets = new(StringComparer.OrdinalIgnoreCase)
    {
    ...
    public SdkConversionVerdict Propose(ProjectModernisation project, string rootPath)
    {
        ...
        // whether it can port is a question about its packages. They are
        // independent, and refusing the first because of the second turned down
        // most of the mechanical work available.
        //
        // nopCommerce 3.90 to 4.00 settles it: the team put all twenty-six
        // projects into the SDK format and left them on .NET Framework, EF6 and
        // all. This tool offered three, because twenty-eight were "blocked by a
        // package with no path forward". Nop.Web was one of them, and in 4.00 it
    ...
    {
        // A target with no steps has no steps to lose. Visual Studio writes an
        // empty `BeforeBuild` and `AfterBuild` into every pre-SDK project ever
        // made, and NuGet writes a guard for imports that PackageReference
        // removes. Counting those as custom build logic refused the mechanical
        // work on almost everything: on nopCommerce 3.90, 61 of the 94 targets
        // were empty or commented out, and 26 of the 31 projects were turned
        // down over them while the team that migrated it converted all 26.
        var targets = root.Elements(ns + "Target")
            .Where(target => !GeneratedTargets.Contains(target.Attribute("Name")?.Value ?? string.Empty))
            .Where(HasSteps)
            .Count();
        ...
        // An import reaching into the packages folder is a packages.config
        // artefact: the package brought its own build targets and the project
        // file was edited to point at them. PackageReference brings them in on
        // its own, so there is nothing to lose. On nopCommerce 3.90 that was 26
        // of the 30 non-standard imports, every one of them the same file.
        var imports = root.Elements(ns + "Import")
            ...
            .Where(path => !FromPackagesFolder(path))
            .ToList();
    ...
    /// <summary>Whether a target actually does anything, comments aside.</summary>
    private static bool HasSteps(XElement target) =>
        target.Elements().Any() || target.Nodes().OfType<XText>().Any(t => t.Value.Trim().Length > 0);

    /// <summary>
    /// Whether an import points into the packages folder a packages.config
    /// restore fills.
    /// </summary>
    private static bool FromPackagesFolder(string path) =>
        path.Replace('\\', '/').Contains("/packages/", StringComparison.OrdinalIgnoreCase)
